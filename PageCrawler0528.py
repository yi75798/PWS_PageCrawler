...
def PostInformation(soup) -> pd.DataFrame:
    ...
    try:
        ...
        if Comment[-1] == "萬":
            Comment = int(re.sub(r"\D","", Comment)) * 1000
        else:
    ...
    try:
        ...
        if Share[-1] == "萬":
            Share = int(re.sub(r"\D","", Share)) * 1000
        else:
            Share = int(re.sub(r"\D","", Share)) * 1000
    except:
        Share = ""
...
### 7.輸出為csv及xls檔
# 路徑最後檔案名記得改!!!!!!
Post_Output.to_csv(path_or_buf=u"/Volumes/GoogleDrive/我的雲端硬碟/台大政研/2021程式設計/final_project/Tsai_test.csv", index=False, encoding="utf-8")
Post_Output.to_excel(u"/Volumes/GoogleDrive/我的雲端硬碟/台大政研/2021程式設計/final_project/Tsai_test.xls", index=False, encoding="utf-8")
