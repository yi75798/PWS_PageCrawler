...
### 1.設定要爬的粉專網址
url = "https://www.facebook.com/tsaiingwen" # 粉專網址
ID = url.split(".com/")[1]
...
def PostInformation(soup) -> pd.DataFrame:
    ...
    try:
        ...
        Reaction = int(re.sub(r"\D", "", Reaction)) * 10000 # ç¨reå»é¤æå­é¨åä¸¦éå
    except:
        Reaction = ""
    ## 留言分享數
...
### 7.輸出為csv及xls檔
# 路徑最後檔案名記得改!!!!!!
Post_Output.to_csv(path_or_buf=u"/Volumes/GoogleDrive/我的雲端硬碟/台大政研/2021程式設計/final_project/Tsaiingwen_test.csv", index=False, encoding="utf-8")
Post_Output.to_excel(u"/Volumes/GoogleDrive/我的雲端硬碟/台大政研/2021程式設計/final_project/Tsaiingwen_test.xls", index=False, encoding="utf-8")
